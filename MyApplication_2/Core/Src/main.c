...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  HAL_Delay(500); // Đợi hệ thống ổn định
  playSoundEvent(SOUND_INTRO);             // Phát âm thanh khi khởi động
  HAL_Delay(1000);

  playSoundEvent(SOUND_HIT_ENEMY);         // 1. Người chơi bắn trúng mục tiêu
  HAL_Delay(1000);

  playSoundEvent(SOUND_HIT_BARRIER);       // 2. Người chơi bắn trúng vật cản
  HAL_Delay(1000);

  playSoundEvent(SOUND_ENEMY_HIT_BARRIER); // 3. Mục tiêu bắn trúng vật cản
  HAL_Delay(1000);

  playSoundEvent(SOUND_ENEMY_HIT_PLAYER);  // 4. Mục tiêu bắn trúng người chơi
  HAL_Delay(1000);

  playSoundEvent(SOUND_WIN);               // 5. Chiến thắng
  HAL_Delay(1000);

  playSoundEvent(SOUND_LOSE);              // 6. Thua cuộc
  HAL_Delay(1000);

  playSoundEvent(SOUND_UI_CLICK);          // 7. Nút bấm giao diện
  HAL_Delay(1000);
  /* USER CODE END 2 */
